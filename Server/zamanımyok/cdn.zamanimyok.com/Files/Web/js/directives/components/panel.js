app.style(function($css, $theme, $path) {

    $css.add("panel", "zc-panel");

    $css.panel
        .begin()
            .relativeLeft()
        .save();

});

app.directive("zcPanel", function() {

    return {
        restricts: "E",
        scope: false,
        compile: function($element, $attr) {

            var w = $attr.zpDimens.split(":")[0];
            var h = $attr.zpDimens.split(":")[1];

            if (w == "fill")
                width = "100%";
            else
                width = w + "px";

            if (h == "fill")
                height = "100%";
            else if (h == "wrap")
                height = "";
            else
                height = h + "px";

            if (height == "")
                $element.css({
                    width: width
                }); 
            else
                $element.css({
                    width: width,
                    height: height
                });

            var anchor = $attr.zpAnchor;

            if (anchor == null) return;

            if (anchor.indexOf("center") != -1) {

                $element.css({ 
                    marginLeft: "calc(50% - "  + (w * 0.5) + "px)", 
                    marginRight: "calc(50% - "  + (w * 0.5) + "px)" 
                });

            }
            
            if (anchor.indexOf("right") != -1) {

                $element.css({ marginLeft: "calc(100% - " + w + "px)" });

            }

            if (anchor.indexOf("left") != -1) {

                $element.css({ marginLeft: "0px" });

            }

        }
    }

});